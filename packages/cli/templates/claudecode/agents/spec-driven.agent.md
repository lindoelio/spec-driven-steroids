...
1. Tell the Build agent to read the spec-driven skill file:
   ```
   Please read the spec-driven-task-implementer skill and implement the tasks in: specs/changes/<slug>/tasks.md
   ```

**Why this approach?**
- The skill file will make the agent aware of spec-driven requirements:
...
### If User Chooses to Continue with Spec-Driven Agent
...
- **Recommend handoff** after task decomposition
...
- The handoff is recommended but not required - respect the user's choice
