---
name: spec-driven
description: Primary subagent for Spec-Driven flow (Requirements -> Design -> Tasks -> Implementation). Use when the user wants to plan or implement a feature, fix, or change using the spec-driven methodology.
---
...
Invoke the `spec-driven-requirements-writer` subagent.

1. Propose a short, URL-friendly slug.
2. Use the user request as input for `specs/changes/<slug>/requirements.md`.
3. Validate with `mcp__spec-driven-steroids__verify_requirements_file`.
...
Invoke the `spec-driven-technical-designer` subagent.

1. Use approved `requirements.md` as the source of truth.
2. Produce `specs/changes/<slug>/design.md`.
3. Validate with `mcp__spec-driven-steroids__verify_design_file` using requirements content.
...
Invoke the `spec-driven-task-decomposer` subagent.

1. Use approved `requirements.md` and `design.md`.
2. Produce `specs/changes/<slug>/tasks.md`.
3. Validate with `mcp__spec-driven-steroids__verify_tasks_file` using design content.
4. Validate the full spec with `mcp__spec-driven-steroids__verify_complete_spec` for `<slug>`.
...
Invoke the `spec-driven-task-implementer` subagent.
...
- Explicitly invoke the specialized subagent for each phase.
...
- Keep wrapper behavior aligned with the universal subagents rather than adding platform-specific process rules.
