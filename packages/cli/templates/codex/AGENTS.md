...
### Primary Agent
- **spec-driven** - Main agent for end-to-end Spec-Driven Development workflow

### Commands
- `/spec-driven` - Start Spec-Driven planning at Phase 1 (requirements)
...
You have access to MCP validation tools via the spec-driven-steroids MCP server:
...
2. **Generate Project Guidelines First (Recommended)**:
...
To invoke a command, use the `/` prefix followed by the command name:

```
/spec-driven Add user authentication with OAuth2
/inject-guidelines
```

Commands will automatically route to the appropriate agent and execute the workflow.

## Using Skills

Skills are specialized functions that can be invoked during agent workflows:
...
Skills are typically invoked by the primary agent during phase execution, but can also be called directly for specific tasks.

## Codex-Specific Notes

When running Codex:

1. **Interactive mode**: `codex` - Opens the interactive CLI with full agent support
2. **Non-interactive**: `codex exec "/spec-driven Add feature X"` - Run a single command
3. **With config override**: `codex -c model="o3" exec "/spec-driven ..."` - Override model

The Spec-Driven agent will guide you through all phases with explicit approval gates.
